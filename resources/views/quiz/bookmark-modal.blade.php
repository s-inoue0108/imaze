...
<div class="quiz-modal">
    ...
    <label for="image-modal-{{ $bookmark->quiz->id }}" class="modal cursor-pointer">
        <div class="flex flex-col md:w-3/4 lg:w-1/2">
            <p class="bg-primary p-2 text-xs text-white font-semibold text-center">「{{ $bookmark->quiz->title }}」</p>
            <figure><img src="{{ asset($bookmark->quiz->image_path) }}" class="w-full h-auto" /></figure>
    ...
    <!--解説-->
    <div>
        <input type="checkbox" id="quiz-modal-{{ $bookmark->quiz->id }}" class="modal-toggle" />
        <label for="quiz-modal-{{ $bookmark->quiz->id }}" class="modal cursor-pointer">
            <label class="modal-box relative bg-yellow-400" for="">
                <h3 class="text-black text-center text-lg font-bold quiz-answer-modal">「{{ $bookmark->quiz->title }}」</h3>
                <div class="border-2 border-yellow-100 rounded mt-2"></div>
                <p class="text-lg text-center text-black font-semibold underline underline-offset-8 pt-4">答え：{{ $bookmark->quiz->answer }}</p>
                @if ($bookmark->quiz->explanation === null)
                <p class="text-black text-center py-8">このクイズの解説はありません</p>
                @else
                <p class="text-black py-8">{{ $bookmark->quiz->explanation }}</p>
                @endif
    ...
    <!--削除の確認-->
    <div>
        <input type="checkbox" id="delete-confirm-modal-{{ $bookmark->quiz->id }}" class="modal-toggle" />
        <div class="modal">
            <div class="modal-box relative bg-yellow-400">
                <h3 class="text-black text-center font-bold delete-confirm-modal">「{{ $bookmark->quiz->title }}」を削除しますか？</h3>
                <div class="border-2 border-yellow-100 rounded mt-2"></div>
                <div class="flex justify-center gap-4 pt-4">
                    <label for="delete-confirm-modal-{{ $bookmark->quiz->id }}" class="btn btn-primary">キャンセル</label>
                    <form method="POST" action="{{ route('quiz.destroy', $bookmark->quiz) }}">
                        @csrf
                        @method('delete')
